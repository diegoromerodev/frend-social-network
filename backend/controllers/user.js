...
exports.users_new_post = [
  body('first_name', "First name can't be empty").trim().isLength({ min: 1 }).escape(),
  body('last_name', "Last name can't be empty").trim().isLength({ min: 1 }).escape(),
  body('password', "Password can't be empty").trim().isLength({ min: 1 }).escape(),
...
exports.user_posts_get = (req, res, next) => {
  Post.find().where('author').equals(req.params.userId).exec((err, posts) => {
    if (err) return next(err);
